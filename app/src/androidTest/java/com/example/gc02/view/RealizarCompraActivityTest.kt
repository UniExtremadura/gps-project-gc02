...
class RealizarCompraActivityTest {
    ...
    fun realizarCompraTest(){
        ...
        val textViewName = onView(
            allOf(
                withId(R.id.tvNombreArticulo), withText("Pantalones vaqueros"),
                childAtPosition(
                    allOf(
        ...
        textViewName.check(matches(withText("Pantalones vaqueros")))
        onView(withId(R.id.btComprar)).perform(scrollTo(), click())
        onView(withId(R.id.comprarArtculoButton)).perform(click())

        Assert.assertNull(articleDao.findById1(1))
    }
